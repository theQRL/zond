...
func TestXMSS_FromExtendedSeed(t *testing.T) {
	...
	address := "010400d9f1efe5b272e042dcc8ef690f0e90ca8b0b6edba0d26f81e7aff12a6754b21788169f7f"

	binHexSeed, err := hex.DecodeString(hexseed)
	assert.Nil(t, err)
...
func TestXMSS_Address(t *testing.T) {
	...
	hexseed := "010400589c3e0bf438bcdc5575c9cb367e9dbb96c7182c9ed5fd6711f832b94d1d5782b345f77d71c1f2f4fa4aa609f040b69f"
	address := "010400d9f1efe5b272e042dcc8ef690f0e90ca8b0b6edba0d26f81e7aff12a6754b21788169f7f"
	bytesAddress := []byte{1, 4, 0, 217, 241, 239, 229, 178, 114, 224, 66, 220, 200, 239, 105, 15, 14, 144, 202, 139, 11, 110, 219, 160, 210, 111, 129, 231, 175, 241, 42, 103, 84, 178, 23, 136, 22, 159, 127}
	...
	assert.Equal(t, x.xmss.StrAddress(), address, "%v != %v", x.xmss.StrAddress(), address)
	assert.Equal(t, x.xmss.Address(), bytesAddress, "%v != %v", x.xmss.Address(), bytesAddress)
}
